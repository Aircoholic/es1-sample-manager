<script>
  ...
  async function handleZip() {
    ...
    await exportAsZip(s);
    message = `✓ ZIP with ${s.length} sample(s) downloaded.`;
    exporting = false;
  }
  ...
  async function handleDirectory(mode) {
    ...
    try {
      const count = await writeToDirectory(s);
      message = `✓ ${count} sample(s) saved ${mode === 'card' ? 'to SmartMedia card' : 'to folder'}.`;
    } catch (e) {
      message = e.name === 'AbortError' ? 'Cancelled.' : `Error: ${e.message}`;
    }
    exporting = false;
  }
  ...
  function printSampleList() {
    ...
    const rows = Array.from({ length: ROWS }, (_, i) => {
      ...
      return `<tr class="${i % 2 === 0 ? 'e' : 'o'}">
        <td class="n">${n1}</td>
        <td class="nm">${c1 ? (c1.label || '') : ''}</td>
        <td class="n sep">${n2}</td>
        <td class="nm">${c2 ? (c2.label || '') : ''}</td>
        <td class="n sep">${n3}</td>
        <td class="nm">${c3 ? (c3.label || '') : ''}</td>
      </tr>`;
    }).join('');
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  html, body { width: 210mm; height: 148mm; overflow: hidden; background: white; }
  ...
  header {
    ...
    padding-bottom: 1.5mm; margin-bottom: 2mm; flex-shrink: 0;
  }
  ...
  thead th {
    font-size: 5pt; text-transform: uppercase; letter-spacing: 0.4px;
    color: #888; border-bottom: 0.4pt solid #ccc;
    padding: 0 1.2mm 0.8mm; text-align: left;
  }
  th.n, td.n { width: 14pt; }
  td { padding: 0.42mm 1.2mm; border-bottom: 0.2pt solid #eee; }
  td.n { font-family: 'Courier New', monospace; font-size: 6pt; color: #999; }
  td.nm { font-size: 6.5pt; }
  td.sep, th.sep { border-left: 0.5pt solid #ddd; padding-left: 2.5mm; }
  tr.e { background: #f7f7f7; }
  tr.o { background: #fff; }

  footer {
    flex-shrink: 0; border-top: 0.4pt solid #ddd;
    padding-top: 1.5mm; margin-top: 1.5mm;
    display: flex; justify-content: space-between; align-items: flex-end;
  }
  .notes-label { font-size: 5pt; text-transform: uppercase; letter-spacing: 0.4px; color: #bbb; margin-bottom: 0.4mm; }
...
<div class="page">
  <header>
    ...
    <div class="meta">Roland ES-1 · SmartMedia Sample List<br>${now}</div>
  </header>
  ...
  <footer>
    <div>
      ...
      <div class="notes-text">${cardNotes || ''}</div>
    </div>
...
{#if showPdfModal}
  <div class="modal-backdrop" onclick={() => showPdfModal = false} role="dialog" aria-modal="true" aria-label="Print Sample List">
    <div class="modal" onclick={(e) => e.stopPropagation()}>
      ...
      <p class="modal-desc">
        Opens a print-ready A5 landscape sheet — use <em>Save as PDF</em> in the browser print dialog.
      </p>
      ...
      <div class="modal-fields">
        ...
        <div class="field">
          ...
          <textarea
            ...
            placeholder="e.g. All samples at 44.1kHz · Kicks normalized · Loops at 128 BPM"
            rows="3"
          ></textarea>
        </div>
      </div>
